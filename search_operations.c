...
sc_result operation_search_all_const_pos_output_arc(sc_event *event, sc_addr arg)
{
    ...
    if (!sc_memory_get_arc_end(arg,&quest)){return SC_ERROR_INVALID_PARAMS;};
    if (!get_question_single_param(CLASS_QUESTION_ALL_OUTPUT_CONST_POS_ARC,quest,&param)){return SC_ERROR_INVALID_PARAMS;};
...
sc_result operation_search_all_const_pos_input_arc(sc_event *event, sc_addr arg)
{
    sc_addr quest,param,answer;
    if (!sc_memory_get_arc_end(arg,&quest)){return SC_ERROR_INVALID_PARAMS;};
    if (!get_question_single_param(CLASS_QUESTION_ALL_INPUT_CONST_POS_ARC,quest,&param)){return SC_ERROR_INVALID_PARAMS;};
    answer=sc_memory_node_new(sc_type_node);
...
sc_result operation_search_full_semantic_neighbourhood(sc_event *event, sc_addr arg)
{
    sc_addr quest,param,answer;
    if (!sc_memory_get_arc_end(arg,&quest)){return SC_ERROR_INVALID_PARAMS;};
    if (!get_question_single_param(CLASS_QUESTION_FULL_SEMANTIC_NEIGHBOURHOOD,quest,&param)){return SC_ERROR_INVALID_PARAMS;};

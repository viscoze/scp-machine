...
sc_bool check_coherence(sc_addr element, sc_addr sc_pattern, sc_addr checked_arc, sc_bool output, sc_type_result *result, sc_type_result *inp_result)
{
    ...
    sc_iterator3 *it_pattern_arc = sc_iterator3_f_a_a_new(element, 0, 0);
    ...
    while (SC_TRUE == sc_iterator3_next(it_pattern_arc))
    {
        ...
        if (SC_TRUE == sc_helper_check_arc(sc_pattern, addr2, sc_type_arc_pos_const_perm))
        {
    ...
    it_pattern_arc = sc_iterator3_a_a_f_new(0, 0, element);
    if (it_pattern_arc == nullptr) {return SC_FALSE;}
    while (SC_TRUE == sc_iterator3_next(it_pattern_arc))
    {
    ...
    if (SC_RESULT_OK != sc_memory_get_element_type(element, &element_type)) {return SC_FALSE;}
    if ((sc_type_node & element_type) != sc_type_node)
    {
        sc_addr end, begin;
        if (SC_RESULT_OK != sc_memory_get_arc_begin(element, &begin)) {return SC_FALSE;}
        if (SC_RESULT_OK != sc_memory_get_arc_end(element, &end)) {return SC_FALSE;}

        if (SC_TRUE == check_coherence(begin, sc_pattern, element, SC_TRUE, result, inp_result) &&
            SC_TRUE == check_coherence(end, sc_pattern, element, SC_FALSE, result, inp_result))
            return SC_TRUE;
        else
            return SC_FALSE;
    }
